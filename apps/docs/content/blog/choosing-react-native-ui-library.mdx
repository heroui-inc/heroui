...
HeroUI Native is a strong fit for iOS and Android apps. It is not currently recommended for Expo web; use HeroUI React for web surfaces.

You should also expect to learn the Native setup:

- Install required peer dependencies.
- Configure Uniwind.
- Import `heroui-native/styles`.
- Add the correct `@source` path.
- Wrap your app with `GestureHandlerRootView` and `HeroUINativeProvider`.

That setup is real work. The payoff is that your team gets a coherent UI foundation instead of re-solving the same component problems every sprint.
