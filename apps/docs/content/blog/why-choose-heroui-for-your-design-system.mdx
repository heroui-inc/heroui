...
image: "/images/blog/why-choose-heroui-for-your-design-system.jpg"
---
...
HeroUI combines [React Aria](https://react-spectrum.adobe.com/react-aria/) for accessibility, [Tailwind CSS v4](https://tailwindcss.com/) for styling, and a compound component API that gives you structural control without prop sprawl. This post explains the concrete reasons to choose it — and the honest trade-offs.
...
HeroUI ships three first-party integrations:
...
Unlike MUI (`ThemeProvider`), Chakra UI (`ChakraProvider`), or Mantine (`MantineProvider`), HeroUI requires no provider wrapper. Install the package, import the CSS, and start using components.
...
This simplifies server component trees, reduces nesting, and eliminates a common source of configuration errors.
...
- [Discord](https://discord.gg/heroui) — Community support and announcements
- [X / Twitter](https://x.com/hero_ui) — Updates and releases

## Honest Trade-Offs

No library is perfect for every project. Here's where HeroUI has real limitations:

- **Smaller community.** HeroUI has ~29K GitHub stars and ~120K weekly npm downloads. MUI has ~98K stars and ~6.7M downloads. shadcn/ui has ~104K stars. You'll find fewer Stack Overflow answers and third-party integrations for HeroUI.
- **Focused component set.** HeroUI doesn't ship a rich text editor, carousel, spotlight search, drag-and-drop, or notification system. Mantine covers those with 120+ components and 70+ hooks. With HeroUI, you'll integrate dedicated libraries for specialized needs.
- **Compound component verbosity.** `Modal.Header`, `Modal.Body`, `Modal.Footer` is more JSX than a flat `<Modal title="..." body="..." />` API. Developers coming from simpler component libraries need to adjust.
- **Tailwind requirement.** HeroUI is built for Tailwind CSS v4. If your team uses CSS Modules, Emotion, or styled-components, adopting HeroUI means adopting Tailwind.
- **v3 is a rebuild.** HeroUI v3 is a ground-up rebuild from the NextUI era. Some search results and blog posts still reference NextUI or v2 APIs that no longer apply.

These are real constraints. If any of them are dealbreakers for your project, consider [shadcn/ui](/blog/heroui-vs-shadcn) (code ownership), [MUI](/blog/heroui-vs-mui) (massive ecosystem), [Mantine](/blog/heroui-vs-mantine) (batteries-included), or [Chakra UI](/blog/heroui-vs-chakra-ui) (style props).
