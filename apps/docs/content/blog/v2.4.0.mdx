...
![image](https://github.com/vezham/heroui-cli/assets/96854855/2a5945dd-5187-4e20-81b8-4136e9791dde)

### Upgrade And Remove Select View Optimization

The disabled option(s) will be displayed in the bottom of the list.

![image](https://github.com/vezham/heroui-cli/assets/96854855/c7ddf9e4-3054-4bf1-b8e3-dc2f6226091a)
...
> If you are using it from `@vezham/react`, no changes are required.

The new `cn` utility exported from the `@vezham/theme` package includes `tailwind-merge` to avoid conflicts between tailwindcss classes overrides and includes the config for NextUI custom classes.

If you are using the `cn` utility from the `@vezham/system`, 

```diff-tsx
-import {cn} from "@vezham/system"
```

or `@vezham/system-rsc` package,

```diff-tsx
-import {cn} from "@vezham/system-rsc"
```
