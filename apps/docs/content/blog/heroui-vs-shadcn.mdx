...
| **Community size** | Growing (~29K GitHub stars) | Very large (~104K GitHub stars) |
...
**Honest trade-offs to consider:** HeroUI has a smaller community than shadcn/ui (~29K vs ~104K GitHub stars) and fewer third-party extensions. The component set is focused — you won't find a built-in rich text editor or spotlight search. The compound component pattern (`Card.Header`, `Modal.Body`) is more verbose than flat APIs, which takes adjustment if you're coming from simpler component libraries. HeroUI v3 is a ground-up rebuild, so some older NextUI content in search results may not apply to current APIs.

## When to Choose shadcn/ui

Choose shadcn/ui when:

- **You need maximum code ownership.** If company policy requires all code to be in your repository (not in node_modules), shadcn/ui's model fits.
- **You need heavy visual customization.** If your design diverges significantly from any library's defaults, owning the source code makes divergent changes easier.
- **You have accessibility expertise in-house.** Your team can maintain Radix-based accessibility as you modify components.
- **You want to learn how components work.** Having the source code in your project is educational — you can read and understand every line.
- **You're comfortable with the maintenance trade-off.** You accept that updates are manual and you're willing to invest the time.
...
HeroUI and shadcn/ui are both excellent choices for different reasons. HeroUI gives you a cohesive, accessible, AI-friendly component library that's easy to maintain. shadcn/ui gives you full code ownership with the trade-off of manual maintenance.

The decision usually comes down to one question: **Do you want a maintained package with a consistent design system, or copied source with maximum ownership?**

Most teams building products (SaaS, internal tools, consumer apps) benefit more from HeroUI's approach. Teams building custom design systems or needing pixel-perfect brand control may prefer shadcn/ui.
