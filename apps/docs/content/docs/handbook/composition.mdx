...
HeroUI components are built as compound components. They export multiple parts that work together. All patterns are equivalent. Choose what fits your codebase.
...
```tsx
import { AlertRoot, AlertIcon, AlertContent, AlertTitle, AlertDescription, AlertClose } from '@heroui/react';
...
```tsx
import { Alert, AlertIcon, AlertClose } from '@heroui/react';
...
Compound components provide flexibility to arrange parts as needed, customize each part independently, and add or remove parts freely.
