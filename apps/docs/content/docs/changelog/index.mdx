...
This release introduces three essential new components ([ComboBox](/docs/components/combobox), [Modal](/docs/components/modal), [Dropdown](/docs/components/dropdown)), improves the [Select](/docs/components/select) component API with a ⚠️ **breaking change**, and includes various refinements and bug fixes.
