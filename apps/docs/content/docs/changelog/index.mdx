...
This release introduces five essential new components ([AlertDialog](/docs/components/alert-dialog), [ComboBox](/docs/components/combobox), [Dropdown](/docs/components/dropdown), [Modal](/docs/components/modal), [NumberField](/docs/components/number-field)), improves the [Select](/docs/components/select) component API with a ⚠️ **breaking change**, enhances custom variants and theme compatibility including dark mode and motion preferences, plus various refinements and bug fixes.
