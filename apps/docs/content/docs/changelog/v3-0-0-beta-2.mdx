---
title: v3.0.0-beta.2
description: Three new components (ComboBox, Modal, Dropdown), Select API improvements, and component refinements.
---
...
- `--color-divider-on-surface`: A calculated variable (automatically generated using `color-mix`) that ensures the divider is visible when placed on a surface background. Like other calculated variables, it can be overridden in your theme.
...
```tsx
<div className="bg-surface">
  <div className="h-px w-full bg-divider-on-surface" />
</div>
```
