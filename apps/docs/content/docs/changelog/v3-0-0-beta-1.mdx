...
<VideoPlayer
  ...
  poster="https://heroui-assets.nyc3.cdn.digitaloceanspaces.com/docs/heroui-og_2x.jpg"
  height={396}
/>

The redesign brings:
- New color system that materializes v3's vision and stands out for its uniqueness
...
Display important messages and notifications to users with status indicators. Supports multiple variants (default, accent, success, warning, danger) and can include titles, descriptions, actions, and close buttons.
...
Checkboxes allow users to select multiple items from a list. The Checkbox component works independently or grouped with CheckboxGroup for better form management.
...
One-time password input component for authentication flows. Supports customizable slot counts, validation states, and visual feedback.
...
A listbox displays a list of options and allows users to select one or more items. Supports single and multiple selection modes, sections, and custom rendering.
...
<ListBox aria-label="Options" selectionMode="single">
  ...
  <ListBox.Item id="option2">Option 2</ListBox.Item>
</ListBox>
```

#### Select

A select displays a collapsible list of options and allows users to select one of them. Built on top of Listbox, providing a familiar dropdown interface.
...
<Select aria-label="Choose an option">
  <Select.Button>Select an option</Select.Button>
  <Select.Popover>
    <ListBox>
      <ListBox.Item id="option1">Option 1</ListBox.Item>
      <ListBox.Item id="option2">Option 2</ListBox.Item>
...
A slider allows users to select a value from a range. Supports horizontal and vertical orientations, custom marks, labels, and output display.
...
A base surface component for creating elevated containers. Useful for cards, panels, and other content containers that need consistent styling.
...
<Surface>
  ...
  <Surface.Content>Content goes here</Surface.Content>
</Surface>
```
...
We're working towards a stable release in** Q4 2025**. This beta release brings us significantly closer to that goal with:
