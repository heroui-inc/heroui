...
export default () => (
  <RadioGroup>
    ...
    <Description />
    <Radio value="option1">
      <Radio.Button> {/* The clickable area: control + label */}
        <Radio.Control>
          <Radio.Indicator>
...
<Callout type="warn">
  Place `Label` **inside** `Radio.Button` (it forms the radio's accessible name), and keep
  `Description`/`FieldError` as **siblings outside** `Radio.Button` so they are exposed via
  `aria-describedby` instead of being folded into the name. For card-style options where the whole
  card should be clickable, put everything inside `Radio.Button`.
</Callout>
...
- `.radio__button` - Radio button (RAC label wrapper for control + label text)
