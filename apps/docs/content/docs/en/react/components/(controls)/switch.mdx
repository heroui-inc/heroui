...
```tsx
import { Switch, Label, Description, FieldError } from '@heroui/react';
...
<Callout type="warn">
  Put the `Label` (and the `Switch.Control`) inside `Switch.Button` — it is the clickable
  area and the switch's accessible name. Keep `Description` and `FieldError` as siblings of
  `Switch.Button` so they are announced via `aria-describedby`. For a switch with **no label**
  you can skip `Switch.Button` and place `Switch.Control` directly inside `Switch` — the control
  becomes the clickable target (pass an `aria-label`).
</Callout>
...
- `.switch__button` - Clickable label wrapping the control and label text
...
The clickable `<label>` that wraps the control and label text. Put the `Label` inside it; keep `Description`/`FieldError` as siblings of `Switch.Button`. Optional for switches with no label — render `Switch.Control` directly inside `Switch` (with an `aria-label`) and the control becomes the clickable target.
