...
<Callout type="warning">
  In multiple mode, a bare `<Autocomplete.Value />` may not display plain text. It renders a copy
  of each selected item's `ListBox.Item` children in the trigger, joined with locale-aware
  separators. Items containing only text render as text, but any markup inside the items
  (checkboxes, avatars, indicators) is duplicated into the trigger, once per selected item. Render
  function children are always called with `isSelected: false`, so selection-dependent styling
  will not carry over either.
</Callout>

To display selected values as plain text, use the `selectedText` render prop value, which joins the
`textValue` of each selected item:

```tsx
<Autocomplete.Value>
  {({isPlaceholder, selectedText}) => (isPlaceholder ? "Select items" : selectedText)}
</Autocomplete.Value>
```
