...
**v3:** Compound components: `Checkbox.Button`, `Checkbox.Control`, `Checkbox.Indicator`

### 2. Label Handling

**v2:** Label passed as children directly to `Checkbox`  
**v3:** Label goes inside `Checkbox.Button` (the clickable label); `Description`/`FieldError` are siblings of `Checkbox.Button`
...
```
Checkbox (Root)
  ├── Checkbox.Button (the clickable label)
  │   ├── Checkbox.Control
  │   │   └── Checkbox.Indicator
  │   └── Label
...
2. **Label Handling**: Labels go inside `Checkbox.Button`
