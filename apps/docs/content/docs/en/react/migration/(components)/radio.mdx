...
**v3:** Compound components: `Radio.Button`, `Radio.Control`, `Radio.Indicator`
...
| `description` (on Radio) | — | Use `Description` component as a sibling of `Radio.Button` |
...
RadioGroup (Root)
  ├── Label (optional)
  ├── Radio
  │   ├── Radio.Button (the clickable label)
  │   │   ├── Radio.Control
  │   │   │   └── Radio.Indicator
  │   │   └── Label
...
1. **Component Structure**: Must use compound components (`Radio.Button`, `Radio.Control`, `Radio.Indicator`)
2. **Label Handling**: `label` prop removed - use `Label` component
3. **Description Handling**: `description` prop removed - use `Description` component as a sibling of `Radio.Button`
