...
| AvatarGroup | `AvatarGroup` | Available again — compound `Avatar` children with `max` / `color` / `variant` / `isGrid` / `overlap` (`clip`\|`ring`) and optional `AvatarGroup.Count`; v2 `total` → explicit `Count`; see [AvatarGroup](/docs/react/components/avatar-group) |
...
### 8. AvatarGroup (Available in v3)

**v2:** Dedicated `AvatarGroup` with props like `isBordered`, `max`, and `total`  
**v3:** `AvatarGroup` is available again as a dedicated component. Wrap compound `Avatar` children, inherit `size` / `color` / `variant` via context, and use `max`, `isGrid`, and `overlap` (`"clip"` default, or `"ring"`). v2 `total` is removed — render an explicit `AvatarGroup.Count` child for a known server total (it suppresses the auto count from `max`). `isBordered` is removed; use Tailwind or `overlap="ring"`. See the [AvatarGroup docs](/docs/react/components/avatar-group).
...
<AvatarGroup max={3} size="md">
  ...
  <Avatar>
    <Avatar.Image src="https://example.com/4.jpg" alt="User 4" />
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ...
    <AvatarGroup max={3}>
      ...
      <Avatar>
        <Avatar.Image src="https://example.com/5.jpg" alt="User 5" />
...
<Tabs items={["v2", "v3"]}>
  ...
  <Tab value="v3">
    ...
    <AvatarGroup max={3} overlap="clip">
      <Avatar>
        <Avatar.Image src="https://example.com/1.jpg" alt="User 1" />
        <Avatar.Fallback>U1</Avatar.Fallback>
      </Avatar>
...
11. **AvatarGroup Available**: Use the dedicated `AvatarGroup` component ([docs](/docs/react/components/avatar-group)) with compound `Avatar` children, `max` / `overlap` / `isGrid`, and optional `AvatarGroup.Count` (replaces v2 `total`)
