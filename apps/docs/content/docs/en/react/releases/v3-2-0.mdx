...
Calendar gains week and day views, a reworked year picker, and range demos built on React Aria 1.18. The toggles (`Radio`, `Checkbox`, `Switch`) move to React Aria's `*Field` + `*Button` composition. This release also rolls in the patch fixes shipped alongside React Aria 1.18 (`Table.SortableColumnHeader`, toast and fieldset fixes, scroll and RTL style fixes).

⚠️ **Breaking changes**: `Radio`, `Checkbox`, and `Switch` replace `*.Content` with `*.Button`. See [Breaking Changes](#breaking-changes).
...
### Radio, Checkbox & Switch: `*.Content` → `*.Button`

These toggles now use React Aria's `*Field` + `*Button` composition. The `*.Content` wrapper is removed. Put the control and `Label` inside a new `*.Button` (the clickable label), and move `Description`/`FieldError` out as siblings of `*.Button` so they're exposed via `aria-describedby` instead of the accessible name.
...
| Before | After |
|--------|-------|
| `<X.Content>` wraps `Label` + help text | `<X.Button>` wraps `X.Control` + `Label` |
| `Description` / `FieldError` inside `X.Content` | `Description` / `FieldError` as siblings of `X.Button` |
| `X.Content` part | Removed — use `X.Button` |

**Control-only checkboxes and switches need no changes.** If you render `Checkbox.Control` / `Switch.Control` directly inside the root (no label, e.g. table row selection or icon switches), it keeps working — the control self-wraps into the clickable element.
