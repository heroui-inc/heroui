...
Patch release: [Tabs](/docs/components/tabs) chevron scrolling lands flush on the edges, `tailwind-variants` is upgraded to `3.3.1` to fix variant modifiers falling back to the wrong class after a re-render, and React Aria is upgraded to `1.20.0` (`react-aria@3.51.0`).
