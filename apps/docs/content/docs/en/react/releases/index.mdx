...
Patch release: upgrades to React Aria `1.19.0` (`react-aria@3.50.0`), fixes clear/close buttons accidentally submitting their surrounding form, and prevents visually hidden inputs in `Checkbox`, `Radio`, and `Switch` from causing overflow scroll.
