...
Calendar week and day views, a reworked year picker, and range demos on React Aria 1.18, plus the patch fixes shipped with it. Breaking: `Radio`, `Checkbox`, and `Switch` replace `*.Content` with `*.Button`.
