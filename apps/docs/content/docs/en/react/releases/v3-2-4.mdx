---
title: v3.2.4
description: Patch release that clamps Tabs.ListContainer chevron scrolling to the scrollable range, upgrades tailwind-variants to 3.3.1 to fix variant classes falling back after a re-render, and upgrades React Aria to 1.20.0.
github:
  pull: 6758
---
...
Patch release: [Tabs](/docs/components/tabs) chevron scrolling now lands flush on the edges, `tailwind-variants` is upgraded to `3.3.1` to fix variant modifiers falling back to the wrong class after a re-render, and React Aria is upgraded to `1.20.0` (`react-aria@3.51.0`).
