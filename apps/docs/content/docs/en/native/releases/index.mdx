...
This patch types Input, TextArea, SearchField, and InputGroup refs as native TextInput instances for React Native's Strict TypeScript API, fixes BottomSheet visibility on Android and during container resize, lets Skeleton `variant="none"` skip default layout animations, and refreshes Menu/Popover/Select anchor positions on window resize.

[Read full release notes →](/docs/native/releases/v1-0-10)
...
This patch reworks font weight resolution through Uniwind `font-*` utilities so custom-font apps get correct weights on iOS, fixes SearchField padding when optional slots are omitted, corrects Toast `duration: 0` to auto-hide as documented, and restores Select/Dialog swipe-to-dismiss on Android under react-native-gesture-handler 3.

[Read full release notes →](/docs/native/releases/v1-0-9)
