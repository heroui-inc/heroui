...
By default the install script writes to your home directory, so the skill is available in every project. Add `HEROUI_SKILL_SCOPE=project` and rerun it from a project root to install there instead — useful when only some of your projects use HeroUI Native and you don't want the skill in every agent's context.

```bash
curl -fsSL https://heroui.com/install | HEROUI_SKILL_SCOPE=project bash -s heroui-native
```
