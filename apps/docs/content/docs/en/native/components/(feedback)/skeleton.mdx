...
<Callout type="info">
  `variant="none"` skips the default `FadeIn`/`FadeOut` layout animations. To restore enter/exit animations on a `none` skeleton, pass explicit `animation.entering` and `animation.exiting` values.
</Callout>
