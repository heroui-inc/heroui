...
- **Tab** - Navigate through focusable elements inside the popover
- **Enter/Space** - Activates the trigger element

#### Screen Reader Support

- Supports `aria-label` and `aria-labelledby` for semantic labeling
- Provides semantic association between trigger and popover
- Content outside the popover is hidden from assistive technologies

#### Focus Management

- Focus moves into the popover when opened
- Focus returns to the trigger element when closed
- Focus trap ensures keyboard navigation stays within the popover

#### Important Features

- Automatically positions relative to trigger element
- Flips placement to avoid overlapping browser window edges
- Closes when clicking outside the popover
- Works seamlessly with screen readers and keyboard navigation
