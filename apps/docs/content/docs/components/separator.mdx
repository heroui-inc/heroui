...
- `.separator--on-surface` - Applied when `isOnSurface` is true, uses `bg-divider-on-surface` for better visibility on surface backgrounds
...
| `isOnSurface` | `boolean \| undefined` | `undefined` | Whether the separator is on a surface background. When `undefined`, automatically detects if it's inside a component with `SurfaceContext` (e.g., Card, Alert, Popover, Modal). When `true`, applies `bg-divider-on-surface` for better visibility. |
