...
The ListBox component displays a list of options and allows users to select one or more items. It's built on top of React Aria Components and provides full keyboard navigation, accessibility, and styling capabilities.

### Features

- **Item selection** – Single or multiple selection modes
- **Keyboard navigation** – Full keyboard support with arrow keys, page up/down, home/end, and typeahead
- **Sections** – Group items into sections with headers
- **Disabled items** – Support for disabling individual items
- **Custom indicators** – Customize selection indicators
- **Accessible** – Follows ARIA listbox pattern with full screen reader support
