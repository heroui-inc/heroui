...
- `.tag` - Base tag styles
- `.tag__remove-trigger` - Remove button trigger
...
#### Variant Classes
- `.tag--on-surface` - Tag variant for use on surface components
...
| `isOnSurface` | `boolean` | - | Whether the tag is displayed on a surface component |
| `size` | `"sm" \| "md" \| "lg"` | - | Size of the tag (inherits from TagGroup if not specified) |
| `className` | `string` | - | Additional CSS classes |
