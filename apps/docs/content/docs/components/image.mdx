...
- **loadingImgWrapper**: Applies additional styles to the image wrapper only in the loading state.
- **fallbackImgWrapper**: Applies additional styles to the image wrapper only in the failed state.
...
<APITable
  data={[
    ...
    },
    {
      ...
      type: 'Partial<Record<"img" | "wrapper" | "zoomedWrapper" | "blurredImg" | "loadingImgWrapper" | "fallbackImgWrapper", string>>',
