  ...
  return (
    <Switch>
      {({isSelected}) => (
        <>
          <Switch.Control
            className="h-6 w-9 bg-gray-300 data-[selected=true]:bg-green-500"
            data-selected={isSelected}
          >
            <Switch.Thumb className="bg-white" />
          </Switch.Control>
          <Switch.Label>Custom styled switch</Switch.Label>
        </>
      )}
    </Switch>
  );
...
- `.switch__label` - Switch label text
...
| `isFocusVisible` | `boolean` | Whether the switch should show focus indicator |
...
The Switch component is built using compound components for maximum flexibility:

- `Switch` - The root component that provides context
- `Switch.Control` - The switch track/control element
- `Switch.Thumb` - The movable thumb element
- `Switch.Icon` - Optional icon inside the thumb
- `Switch.Label` - The label for the switch

For grouping switches:
- `SwitchGroup` - The root group component
- `SwitchGroup.Items` - Container for switch items

### Accessibility

The Switch component follows the [WAI-ARIA Switch Pattern](https://www.w3.org/WAI/ARIA/apg/patterns/switch/). Key features:
