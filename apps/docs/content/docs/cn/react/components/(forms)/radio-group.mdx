...
export default () => (
  <RadioGroup>
    ...
    <Description />
    <Radio value="option1">
      <Radio.Button> {/* 可点击区域：control + label */}
        <Radio.Control>
          <Radio.Indicator>
...
<Callout type="warn">
  将 `Label` 放在 `Radio.Button` **内部**（它构成单选项的无障碍名称），并将
  `Description`/`FieldError` 作为 `Radio.Button` **外部的兄弟节点**，使其通过
  `aria-describedby` 关联，而不会被并入名称。对于整张卡片都可点击的卡片式选项，
  可以把所有内容都放进 `Radio.Button`。
</Callout>
