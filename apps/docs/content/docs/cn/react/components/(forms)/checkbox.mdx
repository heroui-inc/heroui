...
<Callout type="warn">
  只把 `Label` 放进 `Checkbox.Button` —— 它是可点击区域，也是 checkbox 的无障碍名称。
  将 `Description` 和 `FieldError` 作为 `Checkbox.Button` 的兄弟节点，这样它们会通过
  `aria-describedby` 朗读，而不会成为名称的一部分。对于**没有标签**的 checkbox（例如表格行选择），
  可以省略 `Checkbox.Button`，直接把 `Checkbox.Control` 放在 `Checkbox` 内 —— 此时控件本身即为可点击目标。
</Callout>
...
- `.checkbox__button` - 包裹控件与标签文本的可点击 label
...
包裹控件与标签文本的可点击 `<label>`。请把 `Label` 放在它内部；`Description`/`FieldError` 作为 `Checkbox.Button` 的兄弟节点。对于仅有控件的 checkbox 可省略 —— 如果直接把 `Checkbox.Control` 放在 `Checkbox` 内（无标签），控件本身即为可点击目标。
