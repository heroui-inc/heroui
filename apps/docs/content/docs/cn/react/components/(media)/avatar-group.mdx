...
- `.avatar-group--clip` - 沿相邻头像轮廓裁切出的透明缝（默认叠放）
...
| `overlap` | `'clip' \| 'ring'` | `'clip'` | 叠放样式：透明缝裁切（默认）或 box-shadow 描边；`isGrid` 时忽略 |
...
- `overlap` 默认为 `"clip"`（透明缝裁切）；使用 `"ring"` 可改为实线描边；与 `isGrid` 同时使用时无效
