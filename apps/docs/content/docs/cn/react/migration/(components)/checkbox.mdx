...
**v3：** 复合组件：`Checkbox.Button`、`Checkbox.Control`、`Checkbox.Indicator`

### 2. 标签处理

**v2：** 标签直接作为 `children` 传入 `Checkbox`  
**v3：** 标签放在 `Checkbox.Button`（可点击的标签）内；`Description` / `FieldError` 是 `Checkbox.Button` 的同级节点
