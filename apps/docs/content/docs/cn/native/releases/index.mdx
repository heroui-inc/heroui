...
本补丁版本将 Input、TextArea、SearchField 与 InputGroup 的 ref 类型改为原生 `TextInput` 实例，以适配 React Native 的 Strict TypeScript API；修复了 Android 上以及容器尺寸变化时 BottomSheet 的显示问题；让 Skeleton 的 `variant="none"` 跳过默认布局动画；并在窗口尺寸变化时刷新 Menu、Popover 与 Select 的锚点位置。

[阅读完整更新说明 →](/docs/native/releases/v1-0-10)
