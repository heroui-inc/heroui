...
Portal（门户）可将子节点渲染到应用中的其他位置，特别适用于需要浮在其他内容之上的组件，例如模态框、覆盖层与弹出层。

## 默认配置

默认情况下，`PortalHost` 已包含在 `HeroUINativeProvider` 中，无需手动添加。Provider 会自动为所有使用 Portal 的组件设置好门户系统。

## 进阶用法

如需自定义门户实现，可直接从 `heroui-native` 导入 `Portal` 与 `PortalHost`：
...
父组件中的状态变化可能导致门户内组件出现意外问题。例如，将文本输入框直接放入门户内时，若父组件触发重渲染，可能会重置输入框的自动建议，或导致其他界面异常。

为避免该问题，请将交互组件（如输入框）的状态保留在门户内部：把门户内容拆分为独立组件，从而隔离父组件重渲染带来的影响。
...
| children  | `React.ReactNode` | 要渲染到门户中的内容                                   |
