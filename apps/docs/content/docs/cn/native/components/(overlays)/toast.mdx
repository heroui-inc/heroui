...
> **说明**：Provider 的完整配置见 [Provider 文档](/docs/native/getting-started/provider#toast-configuration)。
...
| `duration`      | `number \| 'persistent'`                                                                                                          | `4000`  | 自动隐藏毫秒数；`0` 会在下一个 tick 自动隐藏，只有 `'persistent'` 才会让 Toast 一直停留在屏幕上 |
