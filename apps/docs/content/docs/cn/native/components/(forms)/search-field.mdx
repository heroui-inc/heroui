...
- **SearchField.SearchIcon**：*（可选）* 默认放大镜图标，绝对定位在输入左侧；可传入子节点替换默认图标。省略时，`Input` 不会预留左侧空间。
- **SearchField.Input**：包装 `Input` 并应用搜索相关默认行为；自动从上下文读取 `value` 与 `onChangeText`。仅在组合了 SearchIcon 与 ClearButton 时才分别预留左侧与右侧空间。
- **SearchField.ClearButton**：*（可选）* 清除输入的小图标按钮；值为空时自动隐藏；按下时调用上下文的 `onChange("")`。省略时，`Input` 不会预留右侧空间；若已组合但处于隐藏状态（值为空），右侧内边距会保留，以免首次输入时文本发生跳动。
...
省略 `SearchField.SearchIcon` 即可去掉左侧的放大镜及其预留的内边距，`Input` 会自动收回这部分空间。
...
同时省略 `ClearButton` 则会去掉右侧空间。若 `ClearButton` 已组合但处于隐藏状态（值为空），右侧内边距会保留，以免首次输入时文本发生跳动。
