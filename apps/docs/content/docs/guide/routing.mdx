...
[Inertia](https://inertiajs.com) lets you build single-page apps without building an API, using classic server-side routing. Configure HeroUI to use Inertia's router for client-side navigation.

#### App Setup

Render HeroUIProvider at the app root and forward navigation to router.visit.
...
void createInertiaApp({
  ...
  setup({App, el, props}) {
    ...
    root.render(
      <HeroUIProvider navigate={(path, routerOptions) => router.visit(path, routerOptions)}>
        <App {...props} />
      </HeroUIProvider>
    );
  },
...
#### Using routerOptions with Inertia

`routerOptions` are forwarded to `router.visit` as `VisitOptions`.

```tsx
<Button href="/dashboard" routerOptions={{preserveScroll: true, replace: true}}>
  Dashboard
</Button>
```
