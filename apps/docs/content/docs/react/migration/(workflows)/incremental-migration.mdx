---
title: Incremental Migration with Coexistence
...
<Callout type="info">
  **Key Advantage**: Unlike phased migration, incremental coexistence migration allows the project to remain functional during migration. Both v2 and v3 components can coexist temporarily, enabling you to migrate gradually over time.
</Callout>
...
- You need to deploy incremental updates without breaking the application
...
- **Setup Complexity**: Requires more complex initial setup compared to phased migration
...
### Issue: Styling conflicts between v2 and v3

**Solution:** Ensure CSS import order is correct (`tailwindcss` first, then `@heroui/styles`). Check that both CSS systems are properly configured.

### Issue: TypeScript errors with both versions

**Solution:** Avoid importing both versions in the same file. Migrate files completely to v3 before mixing imports.

### Issue: Provider conflicts

**Solution:** v2 components need `HeroUIProvider`, v3 doesn't. You may need conditional provider wrapping or migrate Provider-dependent components together.

### Issue: Bundle size increase

**Solution:** This is expected during coexistence. Bundle size will return to normal once v2 dependencies are removed.
