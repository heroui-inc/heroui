...
See the [Migration MCP Server](/docs/react/migration/mcp-server) guide for setup. Use these MCP tools:

- `get_migration_workflow`: Get migration overview. For incremental migration, pass `migrationType: "incremental"`.
...
- Check component migration guide using `get_component_migration_guides`
