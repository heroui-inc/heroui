---
title: Agent Migration Guide - Phased Migration
description: Phased migration guide for AI assistants helping migrate HeroUI v2 to v3
index: false
---
...
This guide is designed for AI assistants (agents) helping users migrate from HeroUI v2 to v3. It implements a **phased migration approach** that analyzes the project first and migrates components in manageable batches rather than all at once.
...
1. **Phased Migration**: Never migrate all components at once. Large projects require phased migration.
...
For detailed step-by-step instructions, see the [Phased Migration Guide](/docs/react/migration/phased-migration). The guide covers:
...
See the [Phased Migration Guide](/docs/react/migration/phased-migration#step-5-update-component-imports) for detailed component import changes.
...
Use the `get_component_guides` MCP tool to fetch detailed guides for each component.
...
See the [Phased Migration Guide](/docs/react/migration/phased-migration#step-7-update-custom-theme-overrides) for custom theme override migration.

## Styling Migration

See the [Phased Migration Guide](/docs/react/migration/phased-migration#step-9-styling-migration) for detailed styling migration instructions.
...
1. **`analyze-and-plan`**: Analyze the project to identify all HeroUI v2 components and create a phased migration plan. First step in migration. No arguments required.
...
1. **Guide user to invoke `implement-migration` prompt**
   - Prompt includes instructions for phased migration with checkpoints
...
- Still verify incrementally (phased approach)
- Can use any strategy
- Still need to migrate all code before switching deps

### Mixed v2/v3 Usage (Phased Migration)

- **Not possible in phased migration**: v2 and v3 cannot coexist in phased migration approach
...
- Fix errors incrementally (phased approach)
