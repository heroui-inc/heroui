...
- `maxItems` - No longer supported; render all items or implement collapse manually
- `itemsBeforeCollapse` / `itemsAfterCollapse` / `renderEllipsis` - Collapse/ellipsis feature removed with `maxItems`
- `items` prop pattern - Use children instead
- `onAction` - Use `onPress` on individual items if needed
- `isDisabled` - Not on root in v3; handle per item or via styling if needed
- `classNames` / `itemClasses` - Use `className` on `Breadcrumbs` and `BreadcrumbsItem` instead
