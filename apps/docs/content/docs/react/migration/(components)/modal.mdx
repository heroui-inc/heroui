...
**v3:** Built-in trigger or controlled with `isOpen`/`onOpenChange` on `Modal.Backdrop`, or use `useOverlayState` and pass `state` to the root
...
- `size` - In v3 use `size` on `Modal.Container` (xs, sm, md, lg, cover, full); for custom widths use Tailwind
...
6. **Size**: Use `size` on `Modal.Container` (xs, sm, md, lg, cover, full); `radius` and `shadow` removed — use Tailwind
7. **Backdrop**: `backdrop` → `variant` on `Modal.Backdrop` (values: `opaque`, `blur`, `transparent`)
