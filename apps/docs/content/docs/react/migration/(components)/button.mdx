...
The Button component in HeroUI v3 has been simplified with variant-based styling replacing the separate `color` prop. Both `onClick` and `onPress` are supported.
...
2. **Variants Changed**: New variant system (`primary`, `secondary`, `tertiary`, `ghost`, `danger`, `danger-soft`)
3. **isLoading → isPending**: Loading prop renamed
4. **Default Width Changed**: Buttons now use `w-fit` instead of minimum widths - add `min-w-*` classes to match v2 behavior
...
10. **ButtonGroup Available**: ButtonGroup continues to exist with enhanced prop inheritance - see [ButtonGroup migration guide](/docs/react/migration/button-group)
