...
When used inside a [Surface](/docs/components/surface) component, NumberField automatically applies in-surface styling.

<ComponentPreview name="number-field-in-surface" />
...
- `.number-field__group--in-surface` – Applied when used inside a Surface component
...
| `inSurface` | `"transparent" \| "default" \| "secondary" \| "tertiary"` | - | Whether the field is in a surface (auto-detected when inside Surface). |
...
| `inSurface` | `"transparent" \| "default" \| "secondary" \| "tertiary"` | - | Whether the input is in a surface (auto-detected when inside Surface). |
