...
links:
  ...
  storybook: Components/DataDisplay/ColorSwatch
---
...
The ColorSwatch component supports multiple sizes: `xs`, `sm`, `md` (default), `lg`, and `xl`.
...
ColorSwatch can be displayed as a `circle` (default) or `square`.
...
ColorSwatch displays a checkered background pattern to indicate transparency when using colors with alpha values.
