...
Use `isAttached={false}` to separate buttons with gaps instead of connecting them.
...
#### State Attributes
- `[data-attached="true"]` - Attached mode (connected buttons with dividers)
- `[data-attached="false"]` - Detached mode (separated buttons with gaps)
- `[data-hide-separator="true"]` - Hides the separator between buttons
- `[data-size="sm|md|lg"]` - Size propagated to child buttons
...
| `isAttached` | `boolean` | `true` | Whether buttons are visually connected |
| `hideSeparator` | `boolean` | `false` | Whether to hide the separator between buttons |
