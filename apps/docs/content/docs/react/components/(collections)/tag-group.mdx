...
The `selectedVariant` prop controls the visual style of selected tags. Use `"soft"` for a more subtle selection appearance with softer accent colors.

<ComponentPreview
  name="tag-group-selected-variants"
/>
...
<ComponentPreview
  name="tag-group-on-surface"
/>

### On Form Field

<ComponentPreview
  name="tag-group-on-form-field"
/>
...
- `.tag--selected-default` - Default selected state with accent background (applied when selected)
- `.tag--selected-soft` - Soft selected state with softer accent background (applied when selected)
...
| `selectedVariant` | `"default" \| "soft"` | `"default"` |  Visual style for selected tags (use "soft" for subtle accent) |
| `isOnSurface` | `boolean` | - | Whether tags are displayed on a surface component |
| `isOnFormField` | `boolean` | - | Whether tags are displayed within a form component |
