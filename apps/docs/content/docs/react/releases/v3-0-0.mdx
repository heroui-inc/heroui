...
HeroUI started as a React library with nice defaults and Framer Motion animations. v3 is something different: a design system for React and React Native, built on Tailwind CSS v4 and compound components. 75+ web components. 37 native components. Shared design tokens and compound architecture across both, but each platform is built to feel native on its own. APIs and components differ where React and React Native differ.

The web library was rewritten from scratch. New rendering model, new styling system, new component API. No runtime style engine, no provider wrappers, no Framer Motion dependency. CSS variables, CSS transitions, and [React Aria Components](https://react-aria.adobe.com/) for accessibility.

## Design Principles

Three ideas shaped every decision in v3.

**Composition over configuration.** Components expose their internal pieces as real elements. You compose what you need, rearrange parts, swap them out. No prop soup, no render prop escape hatches.

**CSS-first.** Styles live in CSS files, themed through CSS variables, scoped with BEM classes. Tailwind v4's `@theme` directive maps tokens directly. No JavaScript style objects at runtime.

**Accessible from the start.** v2 used React Aria hooks. v3 migrated to [React Aria Components](https://react-aria.adobe.com/), the higher-level component layer. Keyboard navigation, focus management, screen reader announcements, and ARIA attributes are built in, not bolted on.

## Compound Components

In v2, components took flat props: `<Card title="X" description="Y">`. Clean at first. But customization hit walls fast. How do you style just the header? How do you slot an icon between the title and description?

v3 components use compound structure:
...
HeroUI v3 dropped runtime styles entirely. The theming system runs on Tailwind CSS v4's native CSS variable layer with OKLCH colors.

Every design token is a CSS variable:
...
Component styles use BEM naming: `.button`, `.button--primary`, `.button--lg`, `.card__header`, `.accordion__trigger`. Override any component globally through standard CSS:
...
Every HeroUI component uses CSS transitions tied to data attributes. Popovers fade in with `[data-entering]`. Buttons scale on `[data-pressed]`. Accordions expand with `[aria-hidden="false"]`. No JavaScript animation runtime.
...
75+ components covering forms, data display, navigation, overlays, feedback, layout, date and time, and color.
...
HeroUI Native ships alongside HeroUI v3 for the web. Shared design tokens. Shared compound component pattern. Where the platforms diverge, the APIs adapt to feel native on each. Different rendering engine, same mental model.
...
Alongside v3, the pre-release of [HeroUI Pro](https://heroui.pro) is live. Premium components, templates, and AI tooling for both React and React Native.

### Pro Components

Six components beyond the core library: Command palette, Kanban board, Stats dashboard, Filters, Agenda, and DataGrid. Accessibility, animations, and platform edge cases are handled. Available for both web and native.
...
<iframe
  ...
  width="800"
  height="450"
  src="https://embed.figma.com/design/H0xHxBB8qOKjHHAUCsvgHc/HeroUI-Figma-Kit-V3?m=auto&node-id=2912-29668&embed-host=share"
  allowFullScreen
/>
