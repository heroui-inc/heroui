...
Try it now at [heroui.com/themes](/themes).
...
<Callout type="warning">
  The Toast component is currently in preview and some features might not work as expected.
</Callout>
...
**TextArea Support**: Use `InputGroup.TextArea` for multiline text inputs with prefix and suffix elements. The container automatically adjusts its height and aligns elements to the top.
...
**Focus on Decorative Click**: Clicking on prefix or suffix decorative elements now focuses the input field for better UX.

### ButtonGroup Outline Variant

Added a new `outline` variant to [ButtonGroup](/docs/components/button-group) for outlined button groups.
...
- **Checkbox**: Reduced transition delay for checkmark indicator
- **Checkbox**: Increased stroke width and animation speed for better visibility
...
- Fixed Toast close button on mobile devices
- Fixed Toast duplicate container issue by using global container at app root
- Fixed Toast default gap between toasts (updated from 14 to 8)
...
HeroUI now provides direct exports from react-aria-components for easier access to primitives and utilities.
