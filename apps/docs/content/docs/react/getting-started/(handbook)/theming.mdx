...
Switch between light, dark, or any custom theme by using [next-themes](https://github.com/pacocoursey/next-themes) on Next.js, or HeroUI's `useTheme` hook from `@heroui/react`.
...
Then use `useTheme` from `next-themes` to toggle between themes:
...
In case you're using plain React with Vite or Create React App, you can use the `useTheme` hook from `@heroui/react` to switch between themes. It persists the value in `localStorage`, sets `data-theme` and `class` on `<html>` to the selected name (`light`, `dark`, or a custom theme id), and applies the resolved `light` / `dark` class (including when `system` follows `prefers-color-scheme`) or your custom theme.
