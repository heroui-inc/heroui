...
### HeroUI's render prop

Several compound parts are implemented with HeroUI’s internal `dom` helper - for example `Chip`, `Link.Icon`, `Alert` / `Card` subparts, `Checkbox` / `Radio` visual slots, and similar. They accept an optional `render` callback in the same general form as React Spectrum’s DOM render pattern: `(props, renderProps) => ReactElement`. 

Example: Rendering Chip as a button

```tsx
import {Chip} from '@heroui/react';

<Chip<"button"> render={(props) => <button {...props} />}>
  Label
</Chip>
```
