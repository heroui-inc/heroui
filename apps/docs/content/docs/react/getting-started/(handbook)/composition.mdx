...
Use the `render` prop on the following components to render a custom component in place of the default DOM element: `accordion`, `breadcrumbs`, `button`, `checkbox`, `checkbox-group`, `color-area`, `color-field`, `color-slider`, `color-swatch`, `color-swatch-picker`, `combo-box`, `date-field`, `date-picker`, `date-range-picker`, `disclosure`, `form`, `link`, `list-box`, `number-field`, `popover`, `radio-group`, `search-field`, `select`, `separator`, `slider`, `switch`, `tabs`, `tag-group`, `text-field`, `time-field`, `tooltip`.

This accepts a function which receives the DOM props to pass through, and states such as `isPressed` and `isSelected`.
