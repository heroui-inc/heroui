...
<Callout type="info">
  Refer to the [v3 RadioGroup documentation](/docs/components/radio-group) for complete API reference, styling guide, and advanced examples. This guide only focuses on migrating from HeroUI v2.
</Callout>

## Overview

The RadioGroup component in HeroUI v3 maintains similar functionality to v2 but with updated structure to work with the new compound Radio pattern. The group now uses React Aria Components for better accessibility and requires explicit `Label` component instead of `label` prop.

## Structure Changes

### v2: Label Prop Pattern
