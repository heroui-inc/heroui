...
- **Hooks Removed**: v2 component hooks like `useSwitch` or `useInput` are removed - use React Aria hooks instead (v2 hooks were based on React Aria)
...
HeroUI v2 component hooks like `useSwitch` or `useInput` have been removed in v3. These hooks were based on React Aria hooks, so you should use the React Aria equivalents directly. Note that some hooks like `useInput` do not exist in React Aria, so consider using alternatives like `useTextField`.

**Migration Strategy:**
1. Search your codebase for imports from `@heroui/react` that include hook names
2. Check the [React Aria hooks documentation](https://react-spectrum.adobe.com/react-aria/hooks.html) for guidance
3. Replace v2 hook imports with React Aria equivalents
4. Update hook usage if the API differs (React Aria hooks may have slightly different return values)
