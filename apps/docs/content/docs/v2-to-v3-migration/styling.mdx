...
- `text-tiny` → `text-xs`
- `rounded-small` → `rounded-sm` (or `rounded-[8px]` for exact match)
...
2. Update color references: `primary` → `accent`
3. Update muted colors: `foreground-400` → `muted`
...
**Solution:** Replace custom transition utilities with standard Tailwind `transition-*` utilities. Add `duration-250 ease` to match v2's default 250ms duration and ease timing function. Note that v2 utilities transition many specific properties:

- `.transition-background` → `transition-[background] duration-250 ease`
- `.transition-colors-opacity` → `transition-[color,background-color,border-color,text-decoration-color,fill,stroke,opacity] duration-250 ease`
- `.transition-transform-colors-opacity` → `transition-[transform,scale,color,background,background-color,border-color,text-decoration-color,fill,stroke,opacity] duration-250 ease`
...
- [Main Migration Guide](/docs/v2-to-v3-migration)
- [Theming Documentation](/docs/handbook/theming)
- [Styling Guide](/docs/handbook/styling)
- [Component Migration Guides](/docs/v2-to-v3-migration)
