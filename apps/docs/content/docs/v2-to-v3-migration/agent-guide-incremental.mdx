...
<Callout type="info">
  **Key Difference**: Unlike phased migration, incremental coexistence migration allows the project to remain functional during migration. Both v2 and v3 components can coexist temporarily.
</Callout>

## Key Principles

1. **Incremental Component Migration**: Migrate one component at a time, testing each before proceeding
2. **Project Remains Functional**: Unlike phased migration, the project should remain working throughout
...
<include>./index.mdx#incremental-migration-with-coexistence</include>
...
1. **Identify component to migrate**
   ...
   - Use `get_component_guides` MCP tool to fetch component-specific guide
...
3. **Update component code**
   - Follow component migration guide from `get_component_guides` tool
...
## Differences from Phased Migration
...
- Projects using unified `@heroui/react` package (Strategy A can work but phased may be simpler)
...
- `get_styling_migration_guide`: Get styling migration details (use after all components migrated)
- `list_migration_guides`: See available component guides

## Troubleshooting

### Issue: CSS conflicts between v2 and v3

- Verify CSS import order: `tailwindcss` first, then `@heroui/styles`
- Check Tailwind config has v2 plugin
- Ensure both CSS systems are loaded
- Guide user to adjust CSS specificity if needed

### Issue: Component not rendering correctly

- Verify correct import path (Strategy A: `@heroui-v3/react`, Strategy B: `@heroui/react`)
- Check component migration guide using `get_component_guides`
- Ensure component code follows v3 API patterns
- Test in isolation if needed

### Issue: TypeScript errors

- Verify correct package versions
- Check import paths match strategy
- Ensure types are imported from correct package
- May need to update TypeScript config

### Issue: Styling looks wrong after migration

- Check for CSS conflicts
- Verify CSS import order
- Ensure v3 styles are loaded
