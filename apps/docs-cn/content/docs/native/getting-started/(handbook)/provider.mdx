---
title: Provider
description: 配置 HeroUI Native 的文本、动画与 Toast 等全局能力
icon: chevrons-expand-horizontal
---

`HeroUINativeProvider` 是根级 Provider，用于在 React Native 应用中初始化并配置 HeroUI Native，提供全局配置与门户（Portal）管理。
...
Provider 会自动使用 `react-native-safe-area-context` 的 [`SafeAreaListener`](https://appandflow.github.io/react-native-safe-area-context/api/safe-area-listener) 包裹应用。该组件监听安全区与帧变化且尽量不触发重渲染，并通过 `onChange` 将最新 insets 同步给 Uniwind。
