...
**自定义动画样式：** 若某属性被动画占用，可在支持 `animation` 的组件上通过该属性调整。
...
- `buttonClassNames` — 含 `root`、`label` 等函数
- `cardClassNames` — 含 `root`、`header`、`body`、`footer`、`label`、`description` 等
- `chipClassNames` — 含 `root`、`label`
- 其他组件同理……
