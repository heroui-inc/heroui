...
export function ViewOptions({markdownUrl}: {markdownUrl: string}) {
  const items = useMemo(() => {
    ...
    const query = fullMarkdownUrl
      ? `请阅读 ${fullMarkdownUrl}，我想就其中内容提问。`
      : "我想就这份文档提问。";

    return [
      {
        description: "以 Markdown 格式查看本页",
      ...
      {
        description: "在 VS Code 中安装 MCP 服务器",
  ...
  return (
    <ButtonGroup size="md" variant="tertiary">
      ...
      <Dropdown isOpen={isOpen} onOpenChange={setOpen}>
        <Button isIconOnly aria-label="更多选项" size="md" variant="tertiary">
